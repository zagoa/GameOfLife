...
public class Simulation {
    ...
    public Simulation(int width, int heigth, double populationRate) {
        ...
        field = new Field(this.width, this.heigth);
        animals = new ArrayList<>();
        views = new ArrayList<>();

        SimulatorView view = new GridView(this.width, this.heigth);
        //HEALTHY
    ...
    public void simulateOneStep() {
        ...
        for (int j = 0; j < field.getHeight(); j++) {
            for (int i = 0; i < field.getWidth(); i++) {
                ...
                //Si le LivingBeing est malade, incrémente son temps passé de 1
    ...
    public void addTime(){
        ...
        // for (int j = 0; j < field.getHeight(); j++) {
        //    for (int i = 0; i < field.getWidth(); i++) {
         //       System.out.println("i = " + i + ", j = " + j + ", time = " + field.getLivingBeing(i, j).getTime());
        for (int j = 0; j < this.heigth; j++){
            for (int i =0; i < this.width; i++){
                ...
                if ((tmp.getLivingBeing(i, j) != null) && !((tmp.getLivingBeing(i, j).isHealthy()))) {
                    ...
                    // field.getLivingBeing(i,j).changeState();
                }
            }
        }
    }

    public void deleteDeadBeings() {
        for (int j = 0; j < this.heigth; j++) {
            for (int i = 0; i < this.width; i++) {
